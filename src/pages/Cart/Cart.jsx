...
function Cart() {
  ...
  return (
   ...
   <div className={product_list.some(item => cartItems[item._id] > 0) ? "hidden": "empty-cart-message"}>
  {product_list.some(item => cartItems[item._id] > 0) ? <></>:<h1>Your cart is empty</h1>}
  </div>
    <div className={product_list.some(item => cartItems[item._id] > 0) ? "cart" : "hidden"}>
                ...
                >
                  ...
                  <p className="cart-item-mod">
                    ...
                    <img
                      ...
                      className={cartItems[item._id]===3?"hidden":"modify"}
                      src={datas.plus}
                      alt=""
                    />
                  </p>
                  <button onClick={() => removeAll(item._id)} className="delete">
                  ...
                  <p className={cartItems[item._id]>1?"total":"hidden"}>Total: <br className="br"/> ₹{item.price * cartItems[item._id]}</p>
                  </div>
                </div>
                <hr />
              </>
